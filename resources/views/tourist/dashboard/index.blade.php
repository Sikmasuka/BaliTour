...
@php
    ...
    $userPlannedVisits = $user
        ? \App\Models\VisitPlan::with('destination')->where('user_id', $user->id)->latest()->take(3)->get()
        : collect();
    $userPlannedCount = $user ? \App\Models\VisitPlan::where('user_id', $user->id)->count() : 0;
@endphp

@section('content')
    <!-- 1. CALIBRATED NATURE HERO BANNER -->
    <section
        class="mb-6 relative overflow-hidden rounded-3xl bg-gradient-to-br from-forest-900 via-forest-900 to-forest-800 border border-forest-700/50 px-6 py-6 sm:px-8 sm:py-7 text-white shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
            <div class="space-y-1.5">
                <div
                    class="inline-flex items-center gap-2 rounded-full bg-white/10 border border-white/15 px-3 py-0.5 text-xs text-forest-100 font-medium">
    ...
    <section class="mb-6 grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <a href="{{ route('user.explore-places') }}"
            class="group flex items-center justify-between rounded-2xl bg-white border border-mineral-200 p-4 shadow-2xs hover:border-forest-700/40 hover:shadow-xs transition">
            <div>
            ...
            <div
                class="flex h-10 w-10 items-center justify-center rounded-xl bg-forest-50 border border-forest-100 text-forest-800 text-base">
                🏞️
            </div>
        </a>
        ...
        <a href="/user/bookmarks"
            ...
            <div>
                ...
                <p class="mt-1 font-serif text-2xl font-bold text-forest-950">0</p>
            </div>
            <div
                class="flex h-10 w-10 items-center justify-center rounded-xl bg-forest-50 border border-forest-100 text-forest-800 text-base">
                ❤️
        ...
        <a href="/user/booking-history"
            ...
            <div>
                ...
                <p class="mt-1 font-serif text-2xl font-bold text-forest-950">{{ $userPlannedCount }}</p>
            </div>
            <div
                class="flex h-10 w-10 items-center justify-center rounded-xl bg-forest-50 border border-forest-100 text-forest-800 text-base">
                📅
        ...
        <a href="/user/reviews"
            ...
            <div>
                ...
                <p class="mt-1 font-serif text-2xl font-bold text-forest-950">{{ $userReviewsCount }}</p>
            </div>
            <div
                class="flex h-10 w-10 items-center justify-center rounded-xl bg-forest-50 border border-forest-100 text-forest-800 text-base">
                ⭐
        ...
        <div class="lg:col-span-7 space-y-4">
            <div class="rounded-3xl bg-white border border-mineral-200 p-5 sm:p-6 shadow-2xs">
                <div class="flex items-center justify-between pb-3.5 border-b border-mineral-100">
                    ...
                    <a href="/user/booking-history"
                        class="text-xs font-bold text-forest-700 hover:text-forest-950 inline-flex items-center gap-1">
                        View All &rarr;
                    </a>
                </div>

                <div class="mt-4 space-y-3">
                    @forelse($userPlannedVisits as $visit)
                        <div
                            class="flex items-center justify-between gap-3 rounded-2xl border border-mineral-200/80 bg-forest-50/40 p-3.5 hover:bg-forest-50/80 hover:border-forest-300/80 transition">
                            <div class="flex items-center gap-3.5 min-w-0">
                                <div
                                    class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-forest-900 text-white font-bold text-sm shadow-2xs">
                                    🌿
                                </div>
                                <div class="min-w-0">
                                    <p class="font-bold text-xs sm:text-sm text-forest-950 truncate">
                                        {{ $visit->destination?->name ?? 'Destination Visit' }}</p>
                                    <p class="text-xs text-ink-500 flex items-center gap-1.5 mt-0.5">
                                        <span>📅
                                            {{ $visit->planned_date ? $visit->planned_date->format('M d, Y') : 'Scheduled' }}</span>
                                        <span>·</span>
                                        <span
                                            class="text-forest-700 font-semibold">{{ $visit->destination?->formatted_entrance_fee ?? 'Free' }}</span>
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2 shrink-0">
                                <span
                                    class="rounded-full bg-forest-100 text-forest-800 border border-forest-200 px-2.5 py-0.5 text-[10.5px] font-bold">{{ ucfirst($visit->status ?? 'Upcoming') }}</span>
                                @if ($visit->destination)
                                    <a href="{{ route('destinations.show', $visit->destination->slug) }}"
                                        class="rounded-xl bg-forest-900 hover:bg-forest-800 px-3 py-1.5 text-xs font-bold text-white transition shadow-2xs">
                                        View ↗
                                    </a>
                                @endif
                            </div>
                    ...
                    @empty
                        <div
                            class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-mineral-200 bg-cream-50/50 py-10 text-center">
                            <div
                                class="flex h-10 w-10 items-center justify-center rounded-xl bg-cream-100 text-forest-900 ring-1 ring-cream-200">
                                <span class="text-base">📅</span>
                            </div>
                            <p class="mt-3 text-sm font-semibold text-forest-950">No upcoming planned visits</p>
                            <p class="mt-1 max-w-xs text-xs text-ink-500">You haven't scheduled any destination visits yet.
                                Browse places to add them to your itinerary.</p>
                            <a href="{{ route('user.explore-places') }}"
                                class="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-forest-900 px-3.5 py-1.5 text-xs font-semibold text-cream-50 hover:bg-forest-800 transition shadow-2xs">
                                Explore Places
                            </a>
                        </div>
                    @endforelse
                    ...
                    @forelse($featuredDestinations as $dest)
                        <a href="{{ route('destinations.show', $dest->slug) }}"
                            ...
                            <div class="flex items-center gap-3 min-w-0">
                                <div
                                    class="h-12 w-12 shrink-0 overflow-hidden rounded-xl bg-mineral-100 border border-mineral-200/60">
                                ...
                                <div class="min-w-0">
                                    <p
                                        class="font-bold text-xs sm:text-sm text-forest-950 truncate group-hover:text-forest-700 transition">
                                        {{ $dest->name }}
                                    </p>
                            ...
                            <div class="flex flex-col items-end shrink-0">
                                ...
                                <span class="text-[10.5px] font-bold text-sand-500 mt-0.5">★
                                    {{ $dest->average_rating > 0 ? $dest->average_rating : 'New' }}</span>
                            </div>
                        </a>
                    @empty
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
            <div class="flex items-center gap-2">
                ...
                <div>
                    ...
                    <span class="text-ink-600 ml-1">MDRRMO Rescue: <strong class="text-forest-950">(088)
                            333-2140</strong></span>
            ...
            <a href="mailto:tourism@balingasag.gov.ph"
                class="text-xs font-bold text-forest-700 hover:text-forest-950 shrink-0">
