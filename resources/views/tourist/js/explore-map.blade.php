<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // 100% Free, Watermark-Free OpenStreetMap & High-Res Esri Satellite Layer (No API Key Required)
        ...
        // Layer control switcher (bottom right)
        const baseLayers = {
            "🛰️ Satellite Imagery": satelliteTiles,
            "🏔️ Topo & Terrain": topoTiles
        };
      ...
      `,
                ...
                navigator.geolocation.getCurrentPosition(
                    (pos) => {
                        ...
                        userMarker.bindPopup(
                            '<b style="font-family:Inter,sans-serif;">📍 Your Current Location</b>'
                        ).openPopup();
                        ...
                        locateBtn.innerHTML =
                            '<svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><polygon points="3 11 22 2 13 21 11 13 3 11"/></svg><span class="hidden sm:inline">My GPS</span>';
                    },
                    () => {
                        alert(
                            'Could not determine your GPS location. Please allow location permissions in your browser.'
                        );
                        locateBtn.innerHTML =
                            '<svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><polygon points="3 11 22 2 13 21 11 13 3 11"/></svg><span class="hidden sm:inline">My GPS</span>';
                    }
        ...
        function getPlaceIcon(type = '', osmKey = '', osmVal = '') {
            ...
            if (combined.includes('school') || combined.includes('college') || combined.includes('university'))
                return '🏫';
            if (combined.includes('cafe') || combined.includes('coffee')) return '☕';
            if (combined.includes('restaurant') || combined.includes('food') || combined.includes('fast_food'))
                return '🍽️';
            if (combined.includes('hotel') || combined.includes('resort') || combined.includes('guest_house'))
                return '🏨';
            if (combined.includes('beach') || combined.includes('seashore') || combined.includes('coast'))
                return '🏖️';
            if (combined.includes('church') || combined.includes('cathedral') || combined.includes(
                    'place_of_worship')) return '⛪';
            if (combined.includes('falls') || combined.includes('nature') || combined.includes('park'))
                return '🌿';
            if (combined.includes('gym') || combined.includes('sports')) return '💪';
            if (combined.includes('market') || combined.includes('mall') || combined.includes('shop'))
                return '🛍️';
            return '📍';
        }
    ...
    `;
      ...
      `;
            ...
            if (registeredMatches.length > 0) {
                html +=
                    `<div class="px-3 py-1.5 bg-mineral-100/70 text-[10px] font-bold uppercase tracking-wider text-forest-900">BaliTour Verified Spots</div>`;
        ...
        `;
            ...
            if (osmMatches.length > 0) {
                html +=
                    `<div class="px-3 py-1.5 bg-mineral-100/70 text-[10px] font-bold uppercase tracking-wider text-ink-600">Landmarks & Establishments</div>`;
        ...
        function queryTouristPlaces(query) {
            ...
            const photonUrl =
                `https://photon.komoot.io/api/?q=${encodeURIComponent(query)}&lat=8.7455&lon=124.7745&limit=5`;
        ...
        if (liveSearch) {
            liveSearch.addEventListener('input', (e) => {
                ...
                document.querySelectorAll('.destination-card').forEach(card => {
                    ...
                    const matches = name.includes(q.toLowerCase()) || address.includes(q
                        .toLowerCase());
        ...
        document.addEventListener('click', (e) => {
            if (dropdownBox && liveSearch && !dropdownBox.contains(e.target) && e.target !==
                liveSearch) {
                hideTouristDropdown();
            }
        });
