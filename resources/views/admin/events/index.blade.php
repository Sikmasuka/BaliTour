...
@section('content')
    <section class="mb-8 rounded-3xl bg-forest-900 p-8 text-cream-50 sm:p-10">
        ...
        <p class="mt-3 max-w-2xl text-sm leading-relaxed text-cream-100/80">Curate upcoming festivals, tours, and cultural
            programming for visitors.</p>
    </section>
    <section class="grid gap-4 sm:grid-cols-2">
        <article class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-cream-200">
            ...
            <p class="mt-3 text-sm text-ink-600">Set event times, descriptions, and featured activities for the public
                calendar.</p>
        </article>
        <article class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-cream-200">
            ...
            <p class="mt-3 text-sm text-ink-600">Update event listings with photos, location info, availability, and visitor
                notes.</p>
        </article>
    </section>
    ...
    <section class="mt-8">
                    ...
                    <div class="overflow-hidden rounded-3xl shadow-sm ring-1 ring-cream-200">
                        <table class="min-w-full divide-y divide-cream-200">
                            <thead class="bg-white">
                                <tr>
                                    <th scope="col"
                                        class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-ink-900 sm:pl-6">Name
                                    </th>
                                ...
                                <tr>
                                    <td colspan="4" class="py-16 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="mt-3 text-base font-semibold text-ink-900">No events found</p>
                                            <p class="mt-1 max-w-sm text-xs text-ink-500">There are currently no scheduled
                                                events or festivals. Click "Add Event" above to create one.</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
        ...
        <div id="eventsModal" class="relative z-10 hidden" aria-labelledby="panelTitle" role="dialog" aria-modal="true">
            <div id="eventsModalOverlay" class="fixed inset-0 bg-forest-900/40 opacity-0 transition-opacity duration-300"
            ...
            <div class="fixed inset-0 overflow-hidden">
                <div class="absolute inset-0 overflow-hidden">
                    <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
                        <div id="eventsPanel"
                            class="pointer-events-auto w-screen max-w-xl transform translate-x-full transition-transform duration-300 ease-in-out">
                            <div class="flex h-full flex-col bg-cream-50 shadow-2xl">
                                <div class="flex items-center justify-between border-b border-cream-200 bg-white px-6 py-5">
                                    <div>
                                        ...
                                        <h2 id="panelTitle" class="mt-1 font-serif text-2xl font-medium text-forest-900">Add
                                            New Event</h2>
                                    </div>
                                    <button id="closePanelBtn" type="button" aria-label="Close panel"
                                        class="rounded-full p-2 text-ink-600 hover:bg-cream-100">
                                ...
                                <form action="#" method="POST" class="flex-1 space-y-7 overflow-y-auto px-6 py-6">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            <label for="event-name"
                                                class="mb-1.5 block text-sm font-medium text-ink-900">Event Name</label>
                                            <input type="text" name="event-name" id="event-name"
                                                placeholder="e.g. Nyepi Festival"
                                        ...
                                        <div>
                                            <label for="event-date"
                                                class="mb-1.5 block text-sm font-medium text-ink-900">Event Date</label>
                                        ...
                                        <div>
                                            <label for="event-description"
                                                class="mb-1.5 block text-sm font-medium text-ink-900">Description</label>
                                            <textarea rows="4" name="event-description" id="event-description"
                                    ...
                                    <div>
                                        <label for="event-image-label"
                                            class="mb-1.5 block text-sm font-medium text-ink-900">Event Image</label>
                                        <div id="imagePreviewContainer" class="hidden w-full">
                                            <div class="relative mb-2 w-fit">
                                                <img id="imagePreview" src="" alt="Event image preview"
                                                    class="h-40 w-auto rounded-xl object-cover">
                                                <button id="removeImageBtn" type="button" aria-label="Remove image"
                                                    class="absolute -right-2 -top-2 rounded-full bg-white p-1 shadow-md">
                                        ...
                                        <div id="imageUploadBox"
                                            class="flex cursor-pointer items-center justify-center rounded-xl border-2 border-dashed border-cream-300 bg-white px-6 py-10">
                                            <div class="text-center">
                                                <svg class="mx-auto h-12 w-12 text-ink-300" stroke="currentColor"
                                                    fill="none" viewBox="0 0 48 48" aria-hidden="true">
                                                    <path
                                                        d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.5-3.5a4 4 0 00-5.66 0L17 30.34m0 0l-3.5-3.5a4 4 0 00-5.66 0l-2.83 2.83"
                                                        stroke-width="2" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                </svg>
                                                <p class="mt-3 text-sm text-ink-600"><span
                                                        class="font-semibold text-forest-700">Click to upload</span> or
                                                    drag and drop</p>
                                                <p class="text-xs text-ink-500">PNG, JPG, GIF up to 10MB</p>
                                                <input id="event-image" name="event-image" type="file" class="hidden"
                                                    accept="image/*">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div
                                        class="flex items-center justify-between gap-3 border-t border-cream-200 bg-white px-6 py-4">
                                        <button id="cancelModalBtn" type="button"
                                            class="rounded-2xl border border-cream-200 bg-white px-5 py-2.5 text-sm font-semibold text-ink-900 hover:bg-cream-100">Cancel</button>
                                        <button type="submit"
                                            class="rounded-2xl bg-forest-900 px-5 py-2.5 text-sm font-semibold text-cream-50 shadow-sm hover:bg-forest-700">Save
                                            Event</button>
                                    </div>
                                </form>
                            </div>
    ...
    <script>
        (function() {
            ...
            function closePanel() {
                ...
                eventsPanel.addEventListener('transitionend', handleTransitionEnd, {
                    once: true
                });
            }
