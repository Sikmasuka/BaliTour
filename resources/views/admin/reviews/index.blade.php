...
@section('content')
    <section
        class="mb-8 flex flex-col gap-4 rounded-3xl bg-forest-900 p-8 text-cream-50 sm:flex-row sm:items-center sm:justify-between sm:p-10">
        <div>
            ...
            <p class="mt-3 max-w-2xl text-sm leading-relaxed text-cream-100/80">Moderate traveler feedback and keep public
                reviews trustworthy.</p>
        </div>
    </section>
    ...
    <section
        class="mb-6 flex flex-col gap-3 rounded-3xl bg-white p-4 shadow-sm ring-1 ring-cream-200 sm:flex-row sm:items-center sm:justify-between">
        <div class="relative w-full sm:max-w-sm">
            <svg class="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-400"
        ...
        <div class="flex flex-wrap items-center gap-2">
            <select id="statusFilter"
                class="rounded-2xl border border-cream-200 bg-cream-50 px-3.5 py-2.5 text-sm text-ink-900 focus:border-forest-700 focus:bg-white">
            ...
            <button id="clearFiltersBtn" type="button"
                class="rounded-2xl bg-forest-900 px-4 py-2 text-sm font-semibold text-cream-50 hover:bg-forest-700">Clear
                filters</button>
        </div>
    </section>
    ...
    <div id="emptyState"
        class="hidden flex-col items-center justify-center rounded-3xl border border-dashed border-cream-200 bg-white py-16 text-center">
        <div
        ...
        <p class="mt-4 font-serif text-xl font-medium text-forest-900">No traveler reviews found</p>
        <p class="mt-1 text-sm text-ink-600">When visitors submit reviews for destinations, they will appear here for
            moderation.</p>
    </div>
@endsection
        ...
        (function() {
            ...
            function showToast(message) {
                ...
                if (!toast) {
                    ...
                    toast.id = 'toast';
                    toast.className =
                        'pointer-events-none fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-2xl bg-forest-900 px-5 py-3 text-sm font-medium text-cream-50 opacity-0 shadow-lg transition-all duration-300';
            ...
            function render() {
                ...
                const filtered = items.filter(r => {
                    const matchesSearch = !query || r.author.toLowerCase().includes(query) || r.destination
                        .toLowerCase().includes(query) || r.text.toLowerCase().includes(query);
                ...
                filtered.forEach(review => {
                    ...
                    card.className =
                        'group overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-cream-200 transition hover:shadow-md';
                    card.innerHTML = `
        <div class="p-5">
          ...
          <div class="mt-5 flex flex-wrap gap-2">
            ...
            <button data-delete="${review.id}" class="rounded-2xl border border-red-200 bg-red-100 px-3 py-2 text-sm font-semibold text-red-700 hover:bg-red-200">Delete</button>
                ...
                grid.querySelectorAll('[data-approve]').forEach(btn => btn.addEventListener('click', () =>
                    updateReviewStatus(btn.dataset.approve, 'Approved')));
                grid.querySelectorAll('[data-reject]').forEach(btn => btn.addEventListener('click', () =>
                    updateReviewStatus(btn.dataset.reject, 'Rejected')));
                grid.querySelectorAll('[data-delete]').forEach(btn => btn.addEventListener('click', () => deleteReview(
                    btn.dataset.delete)));
