...
@php
    ...
    $pageSubtitle =
        $subtitle ?? ($__env->yieldContent('page-subtitle') ?: ($isUserPortal ? 'Traveler Portal' : 'Admin Panel'));
...
<div class="w-full max-w-7xl mx-auto px-4 pt-4 pb-1 sm:px-6 sm:pt-6 sm:pb-2 lg:px-8">
    <div class="flex items-center justify-between gap-3 min-w-0">
        ...
        <nav aria-label="Breadcrumb"
            class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-medium text-forest-700 min-w-0 flex-wrap">

            <!-- Mobile Drawer Trigger - Deprecated in favor of modern Mobile Nav Bottom Bar (< 1024px) -->
            {{-- Mobile menu button is hidden as navigation is unified in the bottom bar --}}

            <!-- Root Portal Link -->
            <a href="{{ $homeUrl }}"
                class="inline-flex items-center gap-1.5 font-semibold text-forest-700 hover:text-forest-950 transition-colors shrink-0">
                <svg class="h-4 w-4 text-forest-600" viewBox="0 0 24 24" fill="none" stroke="currentColor"
            ...
            <!-- Separator 1 -->
            <svg class="h-3.5 w-3.5 text-mineral-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"
            ...
            @if (
                !empty($pageSubtitle) &&
                    strtolower($pageSubtitle) !== strtolower($pageTitle) &&
                    strtolower($pageSubtitle) !== strtolower($homeLabel))
                <!-- Subtitle / Section -->
                ...
                <!-- Separator 2 -->
                <svg class="h-3.5 w-3.5 text-mineral-400 shrink-0" viewBox="0 0 24 24" fill="none"
        ...
        <div class="flex items-center gap-2 sm:gap-3 shrink-0">
            <!-- Live date indicator -->
            <div
                class="hidden lg:flex items-center gap-1.5 text-xs font-semibold text-ink-600 bg-white/70 border border-mineral-200/80 px-2.5 py-1 rounded-lg shadow-2xs">
                <svg class="h-3.5 w-3.5 text-forest-700" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            @auth
                ...
                <div class="flex items-center gap-1.5 bg-white border border-mineral-200 rounded-xl p-1 shadow-2xs">
                    ...
                    <div class="flex items-center gap-2 px-1.5 py-0.5"
                        title="Signed in as {{ $authUser->name }} ({{ $authRole }})">
                        <div
                            class="relative flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-forest-900 text-[10px] font-bold text-white shadow-2xs">
                            {{ $authInitials }}
                            <span class="absolute -top-0.5 -right-0.5 flex h-1.5 w-1.5">
                                <span
                                    class="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500 ring-1 ring-white"></span>
                            </span>
                        </div>
                        <span
                            class="hidden sm:inline text-xs font-semibold text-forest-950 max-w-[100px] truncate">{{ $authName }}</span>
                    </div>

                    <div class="h-4 w-px bg-mineral-200"></div>

                    <!-- Header Quick Logout Button -->
                    <button type="button" onclick="if(window.openLogoutModal){ window.openLogoutModal(event); }"
                        ...
                        title="Sign out of account" aria-label="Sign out">
