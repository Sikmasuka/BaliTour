...
@php
    ...
    $pageTitle =
        $title ?? ($__env->yieldContent('title') ?: ($isUserPortal ? 'BaliTours Traveler' : 'Balingasag Admin'));
    $pageSubtitle =
...
<body class="min-h-screen bg-mineral-50 text-ink-900 antialiased selection:bg-forest-100 selection:text-forest-950">
    ...
    <div class="flex min-h-screen">
        <!-- Global Compact Sidebar Navigation (Desktop) -->
        @include('layouts.sidebar', ['portal' => $portal, 'active' => $active])

        <!-- Mobile Navigation Bar & Action Sheets (< 1024px) -->
        <x-mobile-nav :portal="$portal" />

        <!-- Main Content Layout Area -->
        <div
            class="content-wrapper flex min-h-screen {{ View::hasSection('full-bleed') ? 'h-screen max-h-screen overflow-hidden' : '' }} w-full flex-col lg:pl-[250px] pb-24 lg:pb-0 transition-all duration-300 ease-in-out">
